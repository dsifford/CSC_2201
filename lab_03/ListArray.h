...
class List
{
  public:
    ...
    virtual void insert(const T &newDataItem) // Insert after cursor
        throw(logic_error);
    void remove() throw(logic_error);                 // Remove data item
    virtual void replace(const T &newDataItem) // Replace data item
        throw(logic_error);
    ...
    void moveToNth(int n) // Move data item to pos. n
        throw(logic_error);
    bool find(const T &searchDataItem) // Find data item
        throw(logic_error);
