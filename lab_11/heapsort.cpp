...
template<typename T>
void moveDown(T dataItems [], int root, int size) {
	T item;
	int i;
	...
	while (i < size) {
		if (i < size - 1  &&
		    dataItems[i].getPriority() < dataItems[i + 1].getPriority()) {
			i++;
		}
		if (item.getPriority() >= dataItems[i].getPriority()) {
			break;
		}
		else {
